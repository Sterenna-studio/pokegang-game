...
`gang/index.html` is a standalone page served at `pokegang.sterenna.fr/gang/`, hosting the cosmetic panels (music, wallpapers/pins/ball skins, boss title, showcase) that used to live inside the main Gang tab, plus a "vivarium" environment zone (see below) that has grown beyond pure cosmetics. It reuses `state/store.js` to read/write the same `localStorage['pokeforge.v6']` (same origin as the main game) but **never starts the game loop** (`modules/core/tickManager.js` / `startGameLoop()` are not imported there) — this was the whole point of splitting it out, since the cosmetic UI was fighting the live tick loop for re-renders. `gang/gang-app.js` is its boot script; `gang/panels.js` holds the ported panel-render functions; `gang/environment.js` is the vivarium — showcase + boss team Pokémon wandering (periodic CSS-transition repositioning, no animation engine), day/night ambiance and weather overlays, narrative cameo events with dialogue bubbles, and a clickable egg tray that can hatch eggs directly. When touching cosmetic features (`state.cosmetics`, `state.gang.showcase`, `state.gang.titleA/B/C/D`), check whether `gang/panels.js` needs the same change — `modules/ui/gangTab.js` no longer renders any of this itself, only a link card pointing to `/gang/`.

Because two pages can be open at once and both write the same save, `gang/gang-app.js` exposes two separate write paths rather than one "just save everything" function:
- `saveState()` re-reads `localStorage` fresh right before writing and only patches the narrow set of cosmetic fields it owns (`cosmetics`, `gang.showcase`/`titleA-D`/`titleLiaison`/`money`, `purchases`, `activeBall`, `settings.jukeboxTrack`), rather than writing back its full in-memory `state` — this bounds (not eliminates) the risk of clobbering unrelated progress made in the other tab.
- `saveEggHatch(eggId, hatchedPokemon)` backs the vivarium's egg tray, which needs to touch `eggs`/`pokemons`/`stats`/`pokedex` — fields that are very much *not* cosmetic and are actively mutated by the main game. It never copies those collections wholesale; it re-reads fresh and applies the one hatch **additively** (remove this one egg by id, push this one pokemon, increment counters by 1), so a capture made in the other tab in the meantime survives. Any future `/gang/` feature that mutates shared, frequently-changing state should follow this additive pattern rather than widening `saveState()`'s whitelist.

`gang/environment.js`'s egg-hatching reuses the main game's own creation/stat logic rather than reimplementing it — `gang-app.js` imports `modules/core/utils.js` and `modules/systems/pokemon.js` as side-effect-only modules (same pattern as the existing `modules/core/sprites.js`/`modules/ui/cosmetics.js` imports) to get real `makePokemon`/`calculateStats`/`registerPokedexCapture` on `globalThis`, plus the classic-script `data/evolutions-data.js` for `getBaseSpecies`. This works because none of those functions' *own* code paths touch anything `/gang/` doesn't load (e.g. `EVO_BY_SPECIES`-dependent evolution logic is never invoked from a hatch) — if you extend this hatching path, re-check that assumption before calling a new function from `pokemon.js`.
